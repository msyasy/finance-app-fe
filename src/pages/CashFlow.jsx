...
export default function CashFlow() {
  ...
  const fetchTransactions = async () => {
    setLoading(true);
    try {
      const res = await API.get("/transactions?page=1&limit=500").catch(
        () => null,
      );
      if (res?.data) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 p-6 rounded-2xl shadow-sm">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
          Analisis perbandingan mutasi masuk dan keluar untuk memantau surplus
          atau defisit.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 p-5 rounded-2xl shadow-sm">
          ...
          <p
            className={`text-xl font-black mt-1 ${netCashFlow >= 0 ? "text-blue-600 dark:text-blue-400" : "text-rose-600 dark:text-rose-400"}`}
          >
            {netCashFlow >= 0 ? "+" : ""} Rp{" "}
            {netCashFlow.toLocaleString("id-ID")}
          </p>
        </div>
      </div>
        ...
        <div className="pt-8 pb-4 h-64 flex items-end justify-between gap-2 md:gap-6 border-b border-gray-100 dark:border-slate-800">
          {stats.map((s, idx) => {
            const incomeHeight = Math.max(
              Math.round((s.income / maxVal) * 100),
              4,
            );
            const expenseHeight = Math.max(
              Math.round((s.expense / maxVal) * 100),
              4,
            );
